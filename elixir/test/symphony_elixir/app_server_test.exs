defmodule SymphonyElixir.AppServerTest do
  ...
  test "app server emits trace_id in codex session events" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "symphony-elixir-app-server-trace-id-events-#{System.unique_integer([:positive])}"
      )

    try do
      workspace_root = Path.join(test_root, "workspaces")
      workspace = Path.join(workspace_root, "MT-TRACE-ID")
      ...
      issue = %Issue{
        id: "issue-trace-id",
        identifier: "MT-TRACE-ID",
        title: "Trace metadata",
        description: "Ensure AppServer emits trace metadata on events",
        state: "In Progress",
        url: "https://example.org/issues/MT-TRACE-ID",
        labels: ["backend"]
      }
      ...
      assert {:ok, _result} =
               AppServer.run(workspace, "Emit events with trace id", issue,
                 on_message: on_message,
                 trace_id: "trace-app-001"
               )

      assert_receive {:app_server_message, %{event: :session_started, trace_id: "trace-app-001"}}
      assert_receive {:app_server_message, %{event: :turn_completed, trace_id: "trace-app-001"}}
    after
      File.rm_rf(test_root)
    end
  end
