...
GraphQL patterns for Linear API operations. Used during setup (before Symphony
is running) and for post-setup workflows like ticket creation.

## Calling convention

Use `curl` + `$LINEAR_API_KEY`:
...
Parse with `jq`. Responses: `{"data": {...}}` on success, `{"errors": [...]}` on failure.

Keep queries narrowly scoped — ask only for the fields you need.

## Setup operations

### List projects (for project slug discovery)
...
The `slugId` is what goes in `tracker.project_slug` in WORKFLOW.md.
...
Verify custom states (Rework, Human Review, Merging) exist before running Symphony.
